...
import com.formaloo.local.dao.FAQDao
import com.formaloo.local.dao.FAQKeysDao
...
open class FAQListRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            ...
            val endOfPaginationReached = if (force) {
                ...
                val data = body?.data
//                Log.e(TAG, "load:FAQListRemoteMediator ${response.body()}")

                val faqList = arrayListOf<FAQ>()
                data?.rows?.forEach {
                    faqList.add(FAQ(it.slug, Converter().from(it)))
                }
                Log.e(TAG, "faqList:FAQListRemoteMediator ${faqList.size}")
                ...
                withContext(Dispatchers.IO) {
                    ...
                    val faqKeys = faqList.map {
                        FAQKeys(it.faqSlug, prevOffset, nextOffset)
                    }

                    faqKeysDao.saveFAQKeys(faqKeys)
                    faqDao.save(faqList.toList())
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, FAQ>): FAQKeys? {
        // Get the last page that was retrieved, that contained items.
        // From that last page, get the last item
        return state.pages.lastOrNull() { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { faq ->
                // Get the remote keys of the last item retrieved
                faqKeysDao.getFAQKeys(faq.faqSlug)
            }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, FAQ>): FAQKeys? {
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.faqSlug?.let {
                faqKeysDao.getFAQKeys(it)
            }
        }
    }
